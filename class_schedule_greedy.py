...
def create_schedule(subjects, teachers):
    ...
    while uncovered_subjects:
        ...
        for teacher in teachers:
            ...
            # Критерій вибору: максимальне покриття -> мінімальний вік
            if (
                (not best_teacher)
                or (len(can_cover) > len(best_covers))
                or (
                    len(can_cover) == len(best_covers)
        ...
        if not best_teacher:
            # Немає викладача, який може покрити залишкові предмети
            return None

        # Призначаємо викладача
        best_teacher.assigned_subjects = best_covers
        schedule.append(best_teacher)
        uncovered_subjects -= best_covers
